...
export function MobileMenu({ menu }: Props) {
  ...
  return (
    <Sheet onOpenChange={setIsOpen} open={isOpen}>
      <SheetTrigger className="relative flex h-11 w-11 items-center justify-center text-foreground transition-opacity hover:opacity-70">
            ...
            <ul className="flex w-full flex-col gap-1">
              {menu.map((item) => (
                <li className="py-2" key={item.id}>
                  <CMSLink {...item.link} appearance="link" className="lowercase tracking-wide text-base" />
                </li>
              ))}
            </ul>
          ) : null}
