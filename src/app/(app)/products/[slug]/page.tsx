...
function RelatedProducts({ products }: { products: Product[] }) {
  ...
  return (
    <div className="py-12">
      <h2 className="mb-6 text-xl font-light tracking-wide lowercase">related works</h2>
      <ul className="flex w-full gap-4 overflow-x-auto pt-1">
        {products.map((product) => (
          <li
            className="aspect-[4/5] w-full flex-none min-[475px]:w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5"
            key={product.id}
          >
            <Link className="relative h-full w-full" href={`/products/${product.slug}`}>
              <GridTileImage media={product.meta?.image as Media} />
            </Link>
          </li>
        ))}
      </ul>
